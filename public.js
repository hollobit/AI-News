...
function saveURL(){const u=new URL(location.href);for(const k of ['q','day','topic','reviewed','limit','id','section'])u.searchParams.delete(k);u.searchParams.set('view',view);for(const [k,v] of [['q',$('search').value],['day',$('day').value],['topic',$('topic').value],['reviewed',$('reviewed').checked?'1':''],['limit',limit>40?String(limit):'']])if(v)u.searchParams.set(k,v);if(u.href!==location.href)history.pushState(null,'',u);}
...
for(const id of ['search','day','topic','reviewed'])$(id).addEventListener(id==='search'?'input':'change',()=>{limit=40;saveURL();render();});$('more').onclick=()=>{limit+=40;saveURL();render();};$('download').onclick=()=>{const a=link('내려받기','site.json');a.download='AI-News-public.json';a.click();};addEventListener('popstate',()=>{restoreFilters=true;render();});load();
})();
